...
class Auxiliar(object):
    ...
    def getSemiMajorAndSemiMinorAxis(self):
        ...
        distanceArea.setEllipsoid(self.iface.mapCanvas().mapRenderer().destinationCrs().ellipsoidAcronym())
    ...
    def getWGS84Coordinates(self, ponto, srs):
    ...
    def getReprojection(self, ponto, crsSrc, crsDest):
        src = QgsCoordinateReferenceSystem(crsSrc, QgsCoordinateReferenceSystem.EpsgCrsId)
        dest= QgsCoordinateReferenceSystem(crsDest, QgsCoordinateReferenceSystem.EpsgCrsId)
        coordinateTransformer = QgsCoordinateTransform(src, dest)
    ...
    def findFusoUTM(self,ponto,srs):
        nomeFuso = ""
        point = self.getWGS84Coordinates(ponto, srs)
        # VERIFICAR FUSO
        ...
        fusoPath = os.path.join(os.path.dirname(__file__), 'Fusos_Brasil', 'Fusos_Brasil.shp')
        fusoSHP = QgsVectorLayer(fusoPath, "fusos", "ogr")
        
        for fuso in fusoSHP.getFeatures():
            if fuso.attribute(u'FUSO') == nomeFuso:
                crsCode = int(fuso.attribute(u'EPSG'))
                print crsCode
                return QgsCoordinateReferenceSystem(crsCode, QgsCoordinateReferenceSystem.EpsgCrsId)
