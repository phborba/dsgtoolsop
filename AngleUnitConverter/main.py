...
from qgis.core import QgsMapLayer, QgsField, QgsExpression
...
class Main:
    ...
    def doWork(self, b):
        ...
        newField = QgsField(newFieldName, QVariant.Double)
        workingLayer.dataProvider().addAttributes([newField])
        workingLayer.updateFields()
        
        idx = workingLayer.fieldNameIndex(newFieldName)
        print idx
        
        expr = QgsExpression(calculatorString + workingField)
        expr.prepare(workingLayer.pendingFields())
        for feat in workingLayer.getFeatures():
            feat[idx] = expr.evaluate(feat)
